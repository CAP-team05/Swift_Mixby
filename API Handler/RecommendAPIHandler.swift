...
class RecommendAPIHandler {
    public static let shared = RecommendAPIHandler()
    let m2s: [String] = ["null", "겨울", "겨울", "봄", "봄", "봄", "여름", "여름", "여름", "가을", "가을", "가을", "겨울", "겨울"]
    private static let recipeHandler = RecipeHandler.shared
    private static let userHandler = UserHandler.shared
    private static let recommendHandler = RecommendHandler.shared
    
    static func testCode() {
    ...
    func refreshDefaultRecommendDTOs(weather: String, id: Int, completion: @escaping () -> Void) {
        ...
        RecommendHandler.shared.deleteAll()
        ...
        serialQueue.sync {
            getRecommend(weather: weather, id: id) { json in
                ...
                for a in arr {
                    ...
                    RecommendHandler.shared.insert(recommend: recommendDTO)
                }
                
                let cnt = RecommendHandler.shared.searchAll().count
